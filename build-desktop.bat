...
rem  Polozhite etot fayl v koren proekta: D:\sreda\build-desktop.bat
rem  Zapusk: dvoynoy klik ili "build-desktop.bat" v cmd.
rem  Klyuch: build-desktop.bat noclient - sobrat exe BEZ klienta VPN.
rem  Okno NE zakryvaetsya nikogda: skript perezapuskaet sebya v cmd /k,
rem  poetomu vidno lyubuyu oshibku, dazhe samuyu ranyuyu.
rem  Priemy, kotorye nelzya ubirat:
rem   * npm install --ignore-scripts - u apps/web v postinstall stoyat
rem     migracii Prisma, a bazy na mashine sborki net. Electron stavim sami.
...
rem     "tunnel podnyat, svyazi s uzlom net". Skript kopiruet vsyu papku.
...
if not exist "apps\desktop\src\main\winTunnel.ts" goto old_tree
...
echo      net svoego app-builder.exe, budet ispolzovan iz node_modules.
...
echo      Takaya sborka podnimet sluzhbu, no ona srazu umret, i v
echo      prilozhenii budet "tunnel podnyat, svyazi s uzlom net".
...
if not exist "apps\desktop\dist\main\winTunnel.js" goto fail
...
call npx electron-builder --win -c.npmRebuild=false --project apps\desktop
...
goto fail
:old_tree
echo V etoy papke ne ta versiya proekta. Nuzhny fayly:
echo   apps\desktop\src\shared\vpnPlan.ts
echo   apps\desktop\src\main\winTunnel.ts
echo   packages\shared\package.json
echo Raspakuyte svezhiy arhiv proekta i sobirayte iz nego.
goto fail
:no_root
