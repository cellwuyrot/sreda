...
exports.detectForeignTunnels = detectForeignTunnels;
exports.foreignTunnelMessage = foreignTunnelMessage;
 ...
 */
async function detectForeignTunnels() {
    if (process.platform !== "win32")
        return detectForeignTunnelsUnix();
    try {
...
async function detectForeignTunnelsUnix() {
    try {
        ...
        for (const line of stdout.split("\n")) {
            ...
            if (!m)
                continue;
            const name = m[1].trim();
            if (name === vpnPlan_1.TUNNEL_NAME || name === "lo")
                continue;
            if (!/^(wg|tun|tap|awg|nordlynx|proton|utun|ppp)\d*/i.test(name))
                continue;
...
function filterForeign(rows) {
    ...
    for (const row of rows) {
        ...
        if (new RegExp(`^${vpnPlan_1.TUNNEL_NAME}(\\s|$)`, "i").test(name))
            continue;
        const haystack = `${name} ${description}`;
        if (ALLOWED.some((re) => re.test(haystack)))
            continue;
...
function foreignTunnelMessage(list) {
    if (list.length === 0)
        return "";
...
//# sourceMappingURL=foreignVpn.js.map