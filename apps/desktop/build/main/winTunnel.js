...
exports.tunnelServiceRunning = tunnelServiceRunning;
exports.windowsLinkVerdict = windowsLinkVerdict;
exports.windowsTunnelUp = windowsTunnelUp;
exports.windowsTunnelDown = windowsTunnelDown;
exports.stableConfExists = stableConfExists;
const node_child_process_1 = require("node:child_process");
const node_fs_1 = require("node:fs");
const node_path_1 = require("node:path");
const node_util_1 = require("node:util");
const vpnPlan_1 = require("../shared/vpnPlan");
const vpnClient_1 = require("../shared/vpnClient");
 ...
 */
async function elevatedScript(script) {
    const encoded = Buffer.from(script, "utf16le").toString("base64");
    const inner = `-NoProfile -NonInteractive -ExecutionPolicy Bypass -EncodedCommand ${encoded}`;
    const outer = `$p = Start-Process -FilePath 'powershell.exe' -ArgumentList ${psQuote(inner)} ` +
    ...
    try {
        ...
        const e = err;
        if (e.killed)
            throw new Error("Команда управления туннелем не завершилась вовремя");
...
function installLogPath() {
    return (0, node_path_1.join)((0, vpnClient_1.stableClientDir)(process.env), "install.log");
}
/** Последние строки журнала — их показываем человеку вместо общей фразы. */
function installLogTail() {
    try {
        const text = (0, node_fs_1.readFileSync)(installLogPath(), "utf8").trim();
        if (!text)
            return "";
...
async function tunnelServiceRunning() {
    for (const name of (0, vpnClient_1.serviceNames)()) {
        try {
...
async function adapterStatus() {
    try {
        const { stdout } = await run("powershell.exe", [
            ...
            "-NonInteractive",
            "-Command",
            `(Get-NetAdapter -Name ${psQuote(vpnPlan_1.TUNNEL_NAME)} -ErrorAction SilentlyContinue).Status`,
 ...
 */
async function windowsLinkVerdict() {
    try {
        const { stdout } = await run("powershell.exe", [
            "-NoProfile",
            "-NonInteractive",
            "-Command",
            `(Get-NetAdapterStatistics -Name ${psQuote(vpnPlan_1.TUNNEL_NAME)} -ErrorAction SilentlyContinue).ReceivedBytes`,
        ], { windowsHide: true, timeout: 15_000 });
 ...
 */
async function waitTunnelReady() {
...
function clientFiles(exePath) {
    const dir = (0, node_path_1.dirname)(exePath);
    const files = [exePath];
    for (const name of vpnClient_1.WIN_CLIENT_OPTIONAL) {
 ...
 * администраторов.
 */
function installScript(exePath, config, targetDir) {
    const files = clientFiles(exePath);
    const targetExe = (0, node_path_1.join)(targetDir, (0, node_path_1.basename)(exePath));
    const targetConf = (0, node_path_1.join)(targetDir, vpnPlan_1.TUNNEL_CONF_FILE);
    ...
    const lines = [
        ...
        `$log = ${psQuote((0, node_path_1.join)(targetDir, "install.log"))}`,
        ...
        `if (Test-Path -LiteralPath $exe) { & $exe /uninstalltunnelservice ${psQuote(vpnPlan_1.TUNNEL_NAME)} *>> $log }`,
        "Start-Sleep -Milliseconds 800",
        ...(0, vpnClient_1.serviceNames)().map((name) => `sc.exe delete ${psQuote(name)} *>> $log`),
        ...(0, vpnClient_1.serviceNames)().map((name) => `$null = (Get-Service -Name ${psQuote(name)} -ErrorAction SilentlyContinue | ` +
        ...
        `Get-NetAdapter -Name ${psQuote(vpnPlan_1.TUNNEL_NAME)} -ErrorAction SilentlyContinue | ` +
            "ForEach-Object { pnputil /remove-device $_.PnPDeviceID *>> $log }",
        "Start-Sleep -Milliseconds 800",
        "L 'stage: copy client'",
    ];
    for (const file of files) {
        lines.push(`try { Copy-Item -LiteralPath ${psQuote(file)} -Destination $dir -Force -ErrorAction Stop } ` +
            `catch { L "copy failed: ${(0, node_path_1.basename)(file)} $($_.Exception.Message)" }`);
    }
    lines.push("L 'stage: write profile'", `[IO.File]::WriteAllBytes($conf, [Convert]::FromBase64String('${confB64}'))`, 
    /* Профиль без клиента ставить некуда: это единственная действительно
       безнадёжная ситуация, и её видно в журнале сразу. */
    "if (-not (Test-Path -LiteralPath $exe)) { L 'client missing in target dir'; exit 4 }", "L 'stage: install service'", `& $exe /installtunnelservice $conf *>> $log`, "$code = $LASTEXITCODE", "L \"installtunnelservice exit: $code\"", 
    ...
    "$ok = $false", "foreach ($i in 1..25) {", `  foreach ($n in @(${(0, vpnClient_1.serviceNames)().map(psQuote).join(", ")})) {`, "    $s = Get-Service -Name $n -ErrorAction SilentlyContinue", "    if ($s -and $s.Status -eq 'Running') { $ok = $true }", "  }", "  if ($ok) { break }", "  Start-Sleep -Milliseconds 600", "}", "L \"service running: $ok\"", `L "adapter: $((Get-NetAdapter -Name ${psQuote(vpnPlan_1.TUNNEL_NAME)} -ErrorAction SilentlyContinue).Status)"`, "if ($ok) { exit 0 } else { exit 3 }");
    return lines.join("\n");
}
/** Установленный в системе клиент, если он есть. */
function systemClient() {
    for (const candidate of (0, vpnClient_1.systemClientCandidates)(process.env)) {
        if ((0, node_fs_1.existsSync)(candidate))
            return candidate;
    }
    return null;
 ...
 */
async function windowsTunnelUp(config, embeddedExe) {
    const targetDir = (0, vpnClient_1.stableClientDir)(process.env);
    ...
    const attempts = [embeddedExe, system].filter((exe) => !!exe && (0, node_fs_1.existsSync)(exe));
    ...
    for (const exe of attempts) {
        try {
            ...
            if (/права администратора/i.test(lastError.message))
                throw lastError;
            continue;
        }
        /* Вердикт даёт состояние системы, а не код возврата клиента. */
        if (await waitTunnelReady()) {
            return (0, node_path_1.join)(targetDir, (0, node_path_1.basename)(exe));
        }
        const tail = installLogTail();
        lastError = new Error(tail ? `${vpnClient_1.WIN_SERVICE_HINT} Журнал: ${tail}` : vpnClient_1.WIN_SERVICE_HINT);
    }
    throw lastError ?? new Error(vpnClient_1.WIN_SERVICE_HINT);
}
/**
 ...
 */
async function windowsTunnelDown(exePath) {
    const targetDir = (0, vpnClient_1.stableClientDir)(process.env);
    const exe = exePath && (0, node_fs_1.existsSync)(exePath) ? exePath : (0, node_path_1.join)(targetDir, "amneziawg.exe");
    const script = [
        "$ErrorActionPreference = 'Continue'",
        `$exe = ${psQuote(exe)}`,
        `if (Test-Path -LiteralPath $exe) { & $exe /uninstalltunnelservice ${psQuote(vpnPlan_1.TUNNEL_NAME)} 2>$null | Out-Null }`,
        "Start-Sleep -Milliseconds 500",
        ...(0, vpnClient_1.serviceNames)().map((name) => `sc.exe delete ${psQuote(name)} 2>$null | Out-Null`),
        /* Профиль с приватным ключом не должен оставаться на диске после выключения. */
        `Remove-Item -LiteralPath ${psQuote((0, node_path_1.join)(targetDir, vpnPlan_1.TUNNEL_CONF_FILE))} -Force -ErrorAction SilentlyContinue`,
        "exit 0",
    ].join("\n");
 ...
 */
function stableConfExists() {
    try {
        const path = (0, node_path_1.join)((0, vpnClient_1.stableClientDir)(process.env), vpnPlan_1.TUNNEL_CONF_FILE);
        return (0, node_fs_1.existsSync)(path) && (0, node_fs_1.readFileSync)(path, "utf8").includes("[Interface]");
    }
    catch {
        return false;
    }
}
//# sourceMappingURL=winTunnel.js.map