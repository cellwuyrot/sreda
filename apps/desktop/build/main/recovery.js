...
exports.markRenderHealthy = markRenderHealthy;
exports.voiceCallActive = voiceCallActive;
exports.setVoiceActive = setVoiceActive;
exports.clearCacheAndReload = clearCacheAndReload;
exports.invalidateCacheOnVersionChange = invalidateCacheOnVersionChange;
exports.watchStaleAssets = watchStaleAssets;
exports.preventDocumentCaching = preventDocumentCaching;
exports.startCacheMaintenance = startCacheMaintenance;
exports.stopCacheMaintenance = stopCacheMaintenance;
exports.watchBlankRender = watchBlankRender;
 ...
 */
async function clearCacheAndReload(win, reason, opts) {
    if (win.isDestroyed())
        return;
    if (!opts?.force && isVoiceActive()) {
        ...
        try {
            ...
            await electron_1.session.defaultSession.clearCodeCaches({ urls: [] });
        }
...
async function invalidateCacheOnVersionChange() {
    ...
    if (!seen)
        return; // первая установка — кеша ещё нет, чистить нечего
 ...
 */
function watchStaleAssets(win, appOrigin) {
    electron_1.session.defaultSession.webRequest.onCompleted({ urls: [`${appOrigin}/_next/static/*`] }, (details) => {
 ...
 * ручной чистки кеша.
 */
function preventDocumentCaching(appOrigin) {
    electron_1.session.defaultSession.webRequest.onBeforeSendHeaders({ urls: [`${appOrigin}/*`] }, (details, callback) => {
 ...
 */
async function rendererLooksAlive(win) {
    if (win.isDestroyed())
        return true;
    try {
...
function startCacheMaintenance(win) {
...
async function runCacheMaintenance(win) {
    if (win.isDestroyed()) {
        stopCacheMaintenance();
        return;
    }
...
function watchBlankRender(win, appOrigin) {
    let timer = null;
    const cancel = () => {
        if (timer) {
            clearTimeout(timer);
