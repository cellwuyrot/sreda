...
exports.setScreenShareActive = setScreenShareActive;
exports.isPipMode = isPipMode;
exports.setPipMode = setPipMode;
...
const windowStore = new electron_store_1.default({
    name: "window-state",
    defaults: {
...
function navigate(pathname) {
    ...
    const safePath = (0, navigation_1.safeInAppPath)(pathname);
    const win = mainWindow;
    if (!win || win.isDestroyed())
        return;
    ...
    if (sameOrigin && !wc.isLoading()) {
        wc.send(constants_1.IPC.NAVIGATE, safePath);
        return;
    }
...
function createMainWindow() {
    ...
    mainWindow = new electron_1.BrowserWindow({
        ...
        webPreferences: {
            ...
            plugins: true,
            // FIX-OVL-THROTTLE: оверлей голосового канала питается состоянием, которое
            // веб-мост (VoiceOverlayBridge) шлёт из renderer'а ЭТОГО, главного окна
            // примерно раз в секунду и при каждом изменении (кто-то заговорил,
            // замьютился и т.п.). Как только пользователь переключается в игру или
            // другое приложение, Chromium считает главное окно скрытым/фоновым и
            // «замораживает» его фоновые таймеры: setInterval деградирует примерно до
            // одного раза в минуту, а requestAnimationFrame останавливается. Из-за
            // этого поток состояния в оверлей фактически прекращался — оверлей застывал
            // на старых данных или вовсе не успевал получить актуальное состояние
            // именно в тот момент, когда он и нужен (TrioZ не в фокусе). Отключаем
            // троттлинг, чтобы мост продолжал слать состояние, пока окно не в фокусе.
    ...
    const startUrl = new URL(constants_1.DEFAULT_START_PATH, appUrl).toString();
    const appOrigin = new URL(appUrl).origin;
    const splashFile = path_1.default.join(__dirname, "../../static/splash/index.html");
    // ── Resilient loading ──────────────────────────────────────────────
    ...
    const scheduleReload = (reason) => {
        if (retryTimer || !mainWindow || mainWindow.isDestroyed())
            return;
    ...
    mainWindow.webContents.on("did-navigate", (_e, url, httpResponseCode) => {
        ...
        if (httpResponseCode >= 500) {
            ...
            retryCount = 0;
            (0, recovery_1.markRenderHealthy)(); // FIX-BLANK: страница ответила нормально
        }
    });
    ...
    (0, recovery_1.watchStaleAssets)(mainWindow, appOrigin);
    (0, recovery_1.watchBlankRender)(mainWindow, appOrigin);
    ...
    (0, recovery_1.preventDocumentCaching)(appOrigin);
    (0, recovery_1.startCacheMaintenance)(mainWindow);
    ...
    mainWindow.webContents.setWindowOpenHandler(({ url }) => {
        if ((0, navigation_1.isExternalUrl)(url, appUrl)) {
            electron_1.shell.openExternal(url).catch(() => undefined);
            return { action: "deny" };
        }
        if ((0, navigation_1.isBlockedInApp)(url, appUrl)) {
            navigate(constants_1.DEFAULT_START_PATH);
            return { action: "deny" };
        }
    ...
    mainWindow.webContents.on("will-navigate", (event, url) => {
        ...
        if ((0, recovery_1.voiceCallActive)() && (0, navigation_1.isSameOriginDocument)(url, appUrl)) {
            event.preventDefault();
    ...
    mainWindow.on("close", (event) => {
        const { minimizeToTray } = (0, config_1.getConfig)();
    ...
    mainWindow.on("minimize", (event) => {
        if (!screenShareActive || isPipMode())
            return;
        event.preventDefault();
        setPipMode(true);
...
function notifyNavigationHeld() {
    const now = Date.now();
    if (now - navigationHeldNotifiedAt < 60_000)
        return;
