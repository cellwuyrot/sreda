...
exports.safeInAppPath = safeInAppPath;
exports.isExternalUrl = isExternalUrl;
exports.isBlockedInApp = isBlockedInApp;
exports.isSameOriginDocument = isSameOriginDocument;
exports.sanitizeAppUrl = sanitizeAppUrl;
...
function isBlockedPath(pathname) {
    ...
    if (p === "/")
        return true;
    return constants_1.BLOCKED_PATHS.some((bp) => p === bp || p.startsWith(`${bp}/`));
}
/** Путь, по которому можно идти: закрытый раздел подменяется началом приложения. */
function safeInAppPath(pathname) {
    return isBlockedPath(pathname) ? constants_1.DEFAULT_START_PATH : pathname;
}
/**
...
function isBlockedInApp(url, appUrl) {
    try {
        const target = new URL(url);
        const base = new URL(appUrl);
 ...
 */
function isSameOriginDocument(url, appUrl) {
    try {
        const target = new URL(url);
        const base = new URL(appUrl);
        if (target.origin !== base.origin)
        ...
        if (path.startsWith("/auth") || path.startsWith("/api") || path.startsWith("/download"))
            return false;
