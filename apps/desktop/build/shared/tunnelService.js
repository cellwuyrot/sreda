...
exports.MAX_CONFIG_LENGTH = exports.TUNNEL_STALE_MS = exports.AGENT_STALE_MS = exports.TUNNEL_FILE = exports.AGENT_FILE = exports.STATUS_FILE = exports.REQUEST_FILE = exports.AGENT_TASK_NAME = void 0;
exports.serviceDir = serviceDir;
exports.serviceStateDir = serviceStateDir;
exports.serviceRequestDir = serviceRequestDir;
exports.isSafeConfigText = isSafeConfigText;
 ...
 */
function serviceDir(platform, env) {
    if (platform === "win32") {
 ...
 * решают ACL).
 */
function serviceStateDir(platform, env) {
 ...
 */
function serviceRequestDir(platform, env) {
    if (platform === "win32") {
        const base = env["ProgramData"] || "C:\\ProgramData";
...
function isSafeConfigText(config) {
    if (typeof config !== "string")
        return false;
    if (config.length === 0 || config.length > exports.MAX_CONFIG_LENGTH)
        return false;
    if (!/\[Interface\]/i.test(config))
        return false;
...
function parseRequest(raw) {
    ...
    if (action !== "up" && action !== "down")
        return null;
    if (action === "down")
        return { id: value["id"], action, config: "", nonce };
    if (!isSafeConfigText(value["config"]))
        return null;
...
function isAgentAlive(heartbeat, now) {
    if (!heartbeat)
        return false;
    const age = now - heartbeat.at;
    /* Отметка из будущего — переведённые часы, а не живой компонент. */
    return age >= -exports.AGENT_STALE_MS && age <= exports.AGENT_STALE_MS;
}
function parseStatus(raw) {
    ...
    if (state !== "running" && state !== "ok" && state !== "error")
        return null;
...
function parseReport(raw) {
    ...
    const at = value["at"];
    if (typeof handshake !== "number" || typeof at !== "number")
        return null;
    return { handshake, at };
}
/**
 ...
 */
function reportVerdict(report, now, freshSeconds) {
    if (!report)
        return "unknown";
    if (Math.abs(now - report.at) > exports.TUNNEL_STALE_MS)
        return "unknown";
    if (report.handshake <= 0)
        return "silent";
...
function newRequestId(random = Math.random) {
