 ...
 */
/**
 * FIX-WINCLIENT: рядом с `amneziawg.exe` службе туннеля нужны его библиотеки
 * драйвера. Раньше в сборку попадал один exe, и на машине без уже
 * установленного клиента служба создавалась, но сразу умирала — то самое
 * «туннель поднят, связи с узлом нет». Имена различаются между сборками
 * клиента, поэтому список широкий: берём всё, что нашлось.
 */
