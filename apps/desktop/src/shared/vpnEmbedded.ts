 ...
 */
export function tunnelGatewayIp(parsed: ParsedWgConfig): string | null {
  const subnet = tunnelSubnetCidr(parsed);
  if (!subnet) return null;
  const base = subnet.split("/")[0];
  if (!base) return null;
  const octets = base.split(".").map((part) => Number(part));
  if (octets.length !== 4 || octets.some((part) => !Number.isFinite(part))) return null;
