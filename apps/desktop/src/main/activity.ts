 ...
 */
const KNOWN_APPS: Array<[string, string]> = [
  // —— Игры ——
...
function push(label: string | null): void {
  getMainWindow()?.webContents.send(IPC.ACTIVITY_CHANGED, label);
}

async function scan(): Promise<void> {
  const procs = await listProcesses();
  if (procs.size === 0) return; // сбой снятия списка — не сбрасываем статус
  const now = Date.now();
  ...
  for (const [proc] of KNOWN_APPS) {
    ...
    if (procs.has(key)) {
      if (!firstSeen.has(key)) firstSeen.set(key, bootScan ? 0 : now);
    } else {
...
export function resendActivity(): void {
  push(last);
}
