...
import { updateButtonView } from "../shared/updateState";
...
type Subscribe = (handler: (state: DesktopUpdateState) => void) => void;

let root: ShadowRoot | null = null;
let buttonEl: HTMLButtonElement | null = null;
let state: DesktopUpdateState = { status: "idle" };
let installFn: (() => void) | null = null;
...
export function initUpdateButton(
  subscribe: Subscribe,
  initial: () => Promise<DesktopUpdateState>,
  install: () => void,
): void {
  installFn = install;
