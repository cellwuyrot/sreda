 ...
 *
 * Поэтому при смене пула затронутые пиры получают адреса заново: те, чей адрес
...
export function nodeTunnel(node: {
  ...
  return {
    ...
    // Параметры прикладываются только когда администратор объявил узел
    // обфусцированным: иначе отчёт мог бы «включить» режим сам по себе.
    obfuscation: node.transport === "OBFUSCATED" ? parseObfuscation(node.obfuscation) : null,
  };
}
