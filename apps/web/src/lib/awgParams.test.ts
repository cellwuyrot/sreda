...
describe("generateAwgParams", () => {
  ...
  /**
   * ИНВАРИАНТ (FIX-AWG-HANG): генератор ВСЕГДА возвращает управление.
   *
   * Вырожденный генератор — не искусственный случай, а именно то, чем
   * проверяют границы значений. Прежняя реализация набирала четыре разных
   * заголовка перебором со случайностью: при постоянном значении множество
   * навсегда оставалось размером в единицу, и вызов не завершался вовсе — без
   * ошибки, без выхода, с занятым процессом. Этот тест на прежнем коде не
   * падает, а зависает, поэтому у него свой предел времени.
   */
  it("ИНВАРИАНТ: с вырожденным генератором завершается и соблюдает границы", { timeout: 2_000 }, () => {
    ...
    expect(awgProblem(generateAwgParams(() => 0.5))).toBe("");
  });

  it("заголовки H1–H4 различны и в границах", () => {
    for (let i = 0; i < 200; i += 1) {
      const { H1, H2, H3, H4 } = generateAwgParams();
      expect(new Set([H1, H2, H3, H4]).size).toBe(4);
      for (const value of [H1, H2, H3, H4]) {
