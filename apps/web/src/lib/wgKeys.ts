...
/**
 * FIX-MTU: MTU туннеля в выдаваемом профиле.
 *
 * То же значение, что и `SAFE_TUNNEL_MTU` в десктоп-оболочке. Здесь оно
 * повторено, а не импортировано, потому что этот файл собирается и для
 * браузера, где кода десктопа нет: тянуть ради одного числа зависимость
 * между пакетами дороже, чем держать его рядом с местом использования.
 */
export const PROFILE_MTU = 1280;
...
export function buildWireGuardConfig(params: {
  ...
  return [
    ...
    `DNS = ${params.dns}`,
    /* FIX-MTU: MTU выставляется всегда. Без него клиент берёт 1420, а на
       линиях с PPPoE/GPON пакеты такого размера в туннеле не проходят: сессия
       встаёт после рукопожатия, и выглядит это как «интернет пропал». */
    `MTU = ${PROFILE_MTU}`,
    "",
    "[Peer]",
