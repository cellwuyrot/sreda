...
export function generateAwgParams(random: () => number = Math.random): Required<AwgParams> {
  ...
  /* FIX-AWG-HANG: четыре РАЗНЫХ заголовка строятся сдвигом от одного, а не
     перебором «добавляй в множество, пока не наберётся четыре».

     Тот перебор не завершался вовсе, если генератор возвращает одно и то же
     значение: множество навсегда остаётся размером в единицу. Вырожденный
     генератор — не выдумка ради красоты: именно им проверяют границы значений
     (`generateAwgParams(() => 0)`), и такая проверка вешала процесс целиком, без
     ошибки и без выхода. Соседние две строки выше об этом прямо и предупреждают —
     «цикл со случайностью может не завершиться», — а здесь он всё-таки стоял.

     Шаги случайные, но ненулевые, поэтому значения и различны по построению, и
     не идут подряд. Верхняя граница с запасом: 1e9 + 3000 много меньше
     допустимого максимума. */
  const h1 = pick(10, 1_000_000_000);
  const h2 = h1 + pick(1, 1000);
  const h3 = h2 + pick(1, 1000);
  const h4 = h3 + pick(1, 1000);
