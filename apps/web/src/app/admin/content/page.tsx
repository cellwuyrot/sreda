...
function BlockEditorForm({ block, onChange }: { block: AboutBlockRow; onChange: (d: Record<string, unknown>) => void }) {
  ...
  switch (block.type) {
    ...
    case 'bento':    return <BentoEditor data={d as BentoData} onChange={onChange as (d: BentoData) => void} />;
    case 'timeline': return <TimelineEditor data={d as TimelineData} onChange={onChange as (d: TimelineData) => void} />;
    case 'team':     return <TeamEditor data={d as TeamData} onChange={onChange as (d: TeamData) => void} />;
    case 'cta':      return <CtaEditor data={d as CtaData} onChange={onChange as (d: CtaData) => void} />;
    default:         return null;
  }
}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-neutral-600 truncate">
                    ...
                    {block.type === 'gallery' && `${((block.data as GalleryData).items ?? []).length} медиафайлов`}
                    {block.type === 'bento' && `${((block.data as BentoData).items ?? []).length} карточек`}
                    {block.type === 'stats' && `${((block.data as StatsData).items ?? []).length} показателей`}
                    {block.type === 'timeline' && `${((block.data as TimelineData).items ?? []).length} этапов`}
                    {block.type === 'team' && `${((block.data as TeamData).members ?? []).length} участников`}
