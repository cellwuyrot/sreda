      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Кнопка 1 — текст">
          <Inp
            ...
            onChange={(v) => upd({ primaryCta: { ...data.primaryCta!, label: v } })}
          />
        </Field>
        <Field label="Кнопка 1 — href">
          <Inp
            value={data.primaryCta?.href}
            onChange={(v) => upd({ primaryCta: { ...data.primaryCta!, href: v } })}
            placeholder="/connect"
          />
        </Field>
        <Field label="Кнопка 2 — текст">
          <Inp
            ...
            onChange={(v) => upd({ secondaryCta: { ...data.secondaryCta, label: v } })}
          />
        </Field>
        <Field label="Кнопка 2 — href">
          <Inp
            value={data.secondaryCta?.href ?? data.secondaryCta?.action}
            onChange={(v) =>
              upd({
                secondaryCta: {
    ...
    <>
      <Field label="YouTube ID">
        <Inp
          ...
          onChange={(v) => upd({ youtubeId: v, url: "" })}
      ...
      <Field label="URL видео (.mp4 / .webm)">
        <Inp
          value={data.url}
          onChange={(v) => upd({ url: v, youtubeId: "" })}
          placeholder="/uploads/about/video.mp4"
        />
      </Field>
      <Field label="Заголовок">
        <Inp value={data.title} onChange={(v) => upd({ title: v })} />
        ...
        <div key={i} className="flex gap-2 items-end mb-2">
          <div className="flex-1">
            <Field label={`Значение ${i + 1}`}>
              <Inp value={it.value} onChange={(v) => updItem(i, { value: v })} />
            </Field>
          </div>
          <div className="flex-[2]">
            <Field label="Подпись">
              <Inp value={it.label} onChange={(v) => updItem(i, { label: v })} />
            </Field>
          </div>
          <button
            className="mb-4 text-red-500 text-xl"
    ...
    <>
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <div
            key={item.id}
            className="flex gap-2 rounded-lg border border-white/07 bg-white/[0.02] p-3"
          >
            ...
            <button className="text-red-500 text-xl self-start" onClick={() => removeItem(item.id)}>
              ×
            </button>
          </div>
        ))}
      ...
      {items.map((it, i) => (
        <div key={it.key} className="mb-3 rounded-xl border border-white/07 bg-white/[0.025] p-3">
          <div className="grid grid-cols-3 gap-2 mb-2">
            <Field label="Иконка">
              <Inp value={it.icon} onChange={(v) => updItem(i, { icon: v })} />
            </Field>
            <Field label="Заголовок">
              <Inp value={it.title} onChange={(v) => updItem(i, { title: v })} />
            </Field>
            <Field label="Цвет (hex)">
              <Inp value={it.color} onChange={(v) => updItem(i, { color: v })} />
            </Field>
          </div>
          <Field label="Описание">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Field label="Href">
              <Inp value={it.href} onChange={(v) => updItem(i, { href: v })} />
            </Field>
            <Field label="Широкая (2 колонки)">
              <button
      ...
      {items.map((it, i) => (
        <div key={i} className="mb-3 rounded-xl border border-white/07 bg-white/[0.025] p-3">
          <div className="grid grid-cols-4 gap-2 mb-2">
            <Field label="Год">
              <Inp value={it.year} onChange={(v) => updItem(i, { year: v })} />
            </Field>
...
function TeamEditor({
  ...
  return (
    <>
        ...
        <div key={m.id} className="mb-3 rounded-xl border border-white/07 bg-white/[0.025] p-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Field label="Аватар URL">
              <Inp
                value={m.avatarUrl}
                onChange={(v) => updMember(i, { avatarUrl: v })}
      ...
      <button
        className="text-xs text-indigo-400 hover:text-indigo-300 mr-4"
        onClick={() =>
          onChange({
            ...data,
            members: [
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <Field label="Текст кнопки &quot;+&quot;">
          <Inp value={data.joinLabel} onChange={(v) => onChange({ ...data, joinLabel: v })} />
        </Field>
        <Field label="Href кнопки">
          <Inp value={data.joinHref} onChange={(v) => onChange({ ...data, joinHref: v })} />
        </Field>
      </div>
    </>
  );
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Кнопка 2 — href">
          <Inp
            ...
            onChange={(v) => upd({ secondaryCta: { ...data.secondaryCta!, href: v } })}
          />
        </Field>
      </div>
    </>
...
// ─── BlockEditorForm — dispatch по типу блока ────────────────────────────────
// onChange намеренно типизирован как (d: unknown) => void, чтобы избежать
// несовместимости Record<string,unknown> ↔ конкретные интерфейсы блоков.
// Внутри кастуем через unknown → нужный тип (безопасно, данные из БД).
  ...
  switch (block.type) {
    case "hero":
      return (
        <HeroEditor
          data={d as HeroData}
          onChange={onChange as unknown as (d: HeroData) => void}
        />
      );
    case "video":
      return (
        <VideoEditor
          data={d as VideoData}
          onChange={onChange as unknown as (d: VideoData) => void}
        />
      );
    case "stats":
      return (
        <StatsEditor
          data={d as StatsData}
          onChange={onChange as unknown as (d: StatsData) => void}
        />
      );
    case "gallery":
      return (
        <GalleryEditor
          ...
          onChange={onChange as unknown as (d: GalleryData) => void}
          blockId={block.id}
        />
      );
    case "bento":
      return (
        <BentoEditor
          data={d as BentoData}
          onChange={onChange as unknown as (d: BentoData) => void}
        />
      );
    case "timeline":
      return (
        <TimelineEditor
          data={d as TimelineData}
          onChange={onChange as unknown as (d: TimelineData) => void}
        />
      );
    case "team":
      return (
        <TeamEditor
          data={d as TeamData}
          onChange={onChange as unknown as (d: TeamData) => void}
        />
      );
    case "cta":
      return (
        <CtaEditor
          data={d as CtaData}
          onChange={onChange as unknown as (d: CtaData) => void}
        />
      );
    default:
      return null;
  ...
  const move = async (id: string, dir: -1 | 1) => {
    ...
    await Promise.all([
      fetch("/api/about-blocks", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: updated[idx].id, position: idx }),
      }),
      fetch("/api/about-blocks", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: updated[newIdx].id, position: newIdx }),
      }),
    ]);
  };
        ...
        <div className="mx-auto flex max-w-4xl items-center gap-4 px-6 py-4">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-500 transition-all"
            >
            ...
            {blocks.map((block, idx) => (
              <div
                ...
                className={`flex items-center gap-3 rounded-2xl border p-4 transition-all ${
                  block.visible
                    ? "border-white/08 bg-white/[0.025]"
                    : "border-white/04 bg-white/[0.01] opacity-50"
                }`}
              >
                {/* Position arrows */}
                <div className="flex flex-col gap-0.5">
                  <button
                    ...
                    className="p-1 text-neutral-700 hover:text-white disabled:opacity-20 transition-colors"
                  >
                    <svg className="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        >
          <div className="relative w-full max-w-2xl mx-4 rounded-2xl border border-white/10 bg-[#0d0f18] p-6 shadow-2xl">
            <div className="mb-6 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold">
                  {BLOCK_LABELS[editBlock.type as BlockType]}
                </h2>
                <p className="text-xs text-neutral-600">ID: {editBlock.id}</p>
              </div>
              <button
                onClick={closeEdit}
                className="text-neutral-600 hover:text-white text-xl transition-colors"
              >
                ×
              </button>
            </div>
            ...
            <div className="mt-6 flex items-center justify-end gap-3 border-t border-white/06 pt-4">
              <button
                onClick={closeEdit}
                className="rounded-xl border border-white/10 px-5 py-2 text-sm text-neutral-500 hover:text-white transition-colors"
              >
                Отмена
              </button>
              <button
          ...
          <div className="w-full max-w-md mx-4 rounded-2xl border border-white/10 bg-[#0d0f18] p-6 shadow-2xl">
            <div className="mb-5 flex items-center justify-between">
              ...
              <button
                ...
                className="text-neutral-600 hover:text-white text-xl"
              >
                ×
              </button>
            </div>
