...
/* ROLE-STRUCT: числа для плиток. Полные списки пользователей редактору не
   выдаются — /api/editor/overview отдаёт только агрегаты. */
interface Stats {
  users: number;
  services: number;
  openAppeals: number;
...
const GROUPS: NavGroup[] = [
  {
    ...
    items: [
      ...
      /* ROLE-STRUCT: «Премиум», «Рассылка», «ИИ-ассистент» и системная сводка
         убраны: это настройки уровня проекта и платные функции — они
         остаются администратору (и закрыты серверным guard-ом раздела). */
      ...
      { title: "Обращения", description: "История обращений, обратная связь", href: "/admin/appeals", icon: <Icon path={<><rect x="3" y="5" width="18" height="14" rx="2" /><path d="m4 7 8 6 8-6" /></>} />, countKey: "openAppeals" },
      { title: "Проекты", description: "Заявки личных кабинетов, прогресс работ", href: "/admin/projects", icon: <Icon path={<><path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7Z" /><path d="m9 13 2 2 4-4" /></>} />, countKey: "activeProjects" }, // FIX-CABINET
  ...
  },
  {
    ...
    label: "Журнал",
    items: [
      { title: "Логи", description: "Ваши действия в панели", href: "/admin/logs", icon: <Icon path={<><path d="M4 5h16M4 12h16M4 19h10" /></>} /> },
    ],
  },
];
...
export default function EditorPage() {
  ...
  const [stats, setStats] = useState<Stats>({ users: 0, services: 0, openAppeals: 0, activeProjects: 0 });
      ...
      .then((data) => {
        if (data?.stats)
          setStats({
            ...
            openAppeals: data.stats.openAppeals ?? 0,
  ...
  const NAV: { id: CategoryId; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "system", label: "Журнал", icon: <Icon path={<><circle cx="12" cy="12" r="3" /><path d="M19 12a7 7 0 0 0-.1-1.4l2-1.5-2-3.4-2.3 1a7 7 0 0 0-2.4-1.4L13.8 2h-3.6l-.4 2.3A7 7 0 0 0 7.4 5.7l-2.3-1-2 3.4 2 1.5A7 7 0 0 0 5 12c0 .5 0 .9.1 1.4l-2 1.5 2 3.4 2.3-1a7 7 0 0 0 2.4 1.4l.4 2.3h3.6l.4-2.3a7 7 0 0 0 2.4-1.4l2.3 1 2-3.4-2-1.5c.1-.5.1-.9.1-1.4z" /></>} /> },
  ];
