...
function SectionTitle({
  ...
  return (
    <h2
      className={`text-4xl md:text-5xl font-black text-neutral-900 dark:text-white leading-[1.08] mb-3 ${className ?? ""}`}
    >
      {children}
    </h2>
  );
  ...
  return (
    ...
    >
      ...
      {data.badge && (
        <motion.div
          {...fadeUp(0.1)}
          className="mb-8 inline-flex items-center gap-2 rounded-full border border-indigo-500/25 bg-indigo-500/10 px-5 py-2"
        >
      ...
      <motion.h1
        ...
        style={{
          background:
            "linear-gradient(135deg,#f0f2ff 0%,#c4b5fd 40%,#67e8f9 80%)",
      ...
      {data.subtitle && (
        <motion.p
          {...fadeUp(0.2)}
          className="mb-4 text-center text-2xl font-light text-white/40"
        >
          {data.subtitle}
        </motion.p>
      )}
      {data.description && (
        <motion.p
          {...fadeUp(0.25)}
          className="mx-auto mb-10 max-w-lg text-center text-lg leading-relaxed text-white/60"
        >
          {data.description}
        </motion.p>
      )}
  ...
  return (
    <section className="px-6 py-4 flex justify-center">
      <motion.div
        ...
        style={{
          boxShadow: "0 0 80px rgba(99,102,241,.2),0 40px 80px rgba(0,0,0,.6)",
        }}
      >
        {data.youtubeId ? (
...
function GalleryBlock({ data }: { data: GalleryData }) {
  ...
  return (
    <section
      ...
      style={{
        background:
          "radial-gradient(ellipse at 90% 50%,rgba(6,182,212,.06) 0%,transparent 55%)",
      }}
    >
      <motion.div {...fadeUp()}>
        ...
        {data.subtitle && (
          <p className="mb-10 text-sm text-neutral-500">{data.subtitle}</p>
        )}
      </motion.div>
      <div
      ...
      >
        {big && (
          <motion.div
            {...fadeUp(0.05)}
            className="row-span-2 overflow-hidden rounded-2xl border border-white/[0.06] bg-neutral-900 relative group"
          >
        ...
        {rest.map((item, i) => (
          <motion.div
            key={item.id}
            {...fadeUp(0.1 + i * 0.05)}
            className="overflow-hidden rounded-2xl border border-white/[0.06] bg-neutral-900 relative group"
          >
            <MediaItem item={item} />
          </motion.div>
        ))}
  ...
  return (
    <>
      {isVideo ? (
        <video
          src={item.url}
          className={`${fill ? "absolute inset-0 h-full w-full" : "h-full w-full"} object-cover`}
          muted
          loop
          playsInline
          autoPlay
      ...
      {isGif && (
        <span className="absolute top-2.5 right-2.5 rounded bg-red-500 px-1.5 py-0.5 text-[9px] font-bold text-white">
          GIF
        </span>
      )}
    </>
  ...
  return (
    <section className="px-6 md:px-10 lg:px-16 py-16">
      <motion.div {...fadeUp()}>
        ...
        {data.title && <SectionTitle>{data.title}</SectionTitle>}
        {data.subtitle && (
          <p className="mb-8 text-sm text-neutral-500">{data.subtitle}</p>
        )}
      </motion.div>
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow: `0 0 20px -6px ${item.color}66`,
                }}
              >
                {item.imageUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
    ...
    <section className="px-6 md:px-10 lg:px-16 py-16">
      ...
      <div className="relative mt-8 flex flex-col pl-6">
        <div
          ...
          style={{
            background:
              "linear-gradient(180deg,rgba(99,102,241,.5),rgba(139,92,246,.3),transparent)",
          }}
        />
        {items.map((item, i) => (
            ...
            <div
              ...
              style={{
                ...
                boxShadow: item.current
                  ? `0 0 10px ${item.color ?? "#6366f1"}`
                  : undefined,
                borderStyle: item.current ? "dashed" : "solid",
              }}
            />
            <div className="min-w-[40px] text-xs font-semibold text-neutral-500 pt-0.5">
              {item.year}
            </div>
            <div>
              <div
                ...
                style={
                  item.current ? { color: item.color ?? "#6366f1" } : undefined
                }
              >
                {item.title}
              </div>
          ...
          >
            {m.avatarUrl ? (
              ...
              <div
                ...
                style={{
                  background: `${m.color ?? "#6366f1"}14`,
                  borderColor: `${m.color ?? "#6366f1"}40`,
                }}
              >
                {m.emoji ?? "\u{1F464}"}
              </div>
...
function CtaBlock({ data }: { data: CtaData }) {
  return (
    <section
      className="mx-6 md:mx-10 lg:mx-16 mb-20 overflow-hidden rounded-3xl border border-indigo-500/22 p-14 text-center relative"
      style={{
        background:
          "linear-gradient(135deg,rgba(99,102,241,.14),rgba(139,92,246,.09))",
      }}
    >
      <div
        className="pointer-events-none absolute inset-0"
        style={{
          background:
            "radial-gradient(ellipse at center,rgba(99,102,241,.1),transparent 70%)",
        }}
      />
      <motion.div {...fadeUp()}>
        <h2 className="relative mb-4 text-5xl font-black text-white">
          {data.title}
        </h2>
        {data.subtitle && (
          <p className="relative mb-9 text-lg text-neutral-500">
            {data.subtitle}
          </p>
        )}
    ...
    >
      ...
      <div className="relative p-6 flex flex-col flex-1">
        ...
        <div className="mb-4 flex items-center gap-3">
          <div
            ...
            style={{
              ...
              background: cfg.bg,
              boxShadow: `0 0 20px -4px ${cfg.color}60`,
            }}
          >
            {cfg.icon}
          </div>
        ...
        <div className="mb-2 flex items-baseline gap-2">
          ...
          {app.version && (
            <span className="rounded bg-white/[0.07] px-1.5 py-0.5 text-[10px] font-mono text-white/50">
              v{app.version}
            </span>
          )}
        </div>
        ...
        {/* Description */}
        {app.description && (
          <p className="mb-4 text-sm leading-relaxed text-neutral-500 flex-1">
            {app.description}
          </p>
        )}
          ...
          {app.fileUrl ? (
            <a
              ...
              style={{
                ...
                boxShadow: `0 0 20px -4px ${cfg.color}80`,
              }}
            >
              <svg
    ...
    <div className="relative min-h-screen overflow-x-hidden text-neutral-900 dark:text-white bg-[#07090f]">
      ...
      {loading ? (
        ...
        <>
          ...
          {blocks.length === 0 && (
            <section id="about-empty" className="px-6 md:px-10 lg:px-16 py-20">
              ...
              <p className="max-w-2xl text-sm leading-relaxed text-neutral-400">
                ...
                видео, статистика, галерея, карточки, история, команда, CTA и
                приложения — настраиваются в Админ → О проекте. Правовая
                информация редактируется отдельно и показывается ниже автоматически.
              </p>
            </section>
          )}
          ...
          {/* Правовая информация — системная часть страницы, а не CMS-блок. */}
          <LegalFooter />
          ...
          <footer className="border-t border-indigo-500/10 px-6 py-8 text-xs text-neutral-700">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                <svg width="14" height="14" viewBox="0 0 100 100" aria-hidden>
                  <g transform="translate(50,52)" fill="#6366f1" opacity=".6">
                    <path
                      d="M5,6 L5,-30 L22,-30 L26,-34 L26,-43 L22,-47 L-22,-47 L-26,-43 L-26,-34 L-22,-30 L-5,-30 L-5,6Z"
                      transform="rotate(0)"
                    />
                    <path
                      d="M5,6 L5,-30 L22,-30 L26,-34 L26,-43 L22,-47 L-22,-47 L-26,-43 L-26,-34 L-22,-30 L-5,-30 L-5,6Z"
              ...
              <span>
                &#169; {new Date().getFullYear()} TRIOZ. Все права защищены.
              </span>
            </div>
          </footer>
        </>
