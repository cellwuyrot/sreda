...
function getAccent(host: string): Accent {
...
function trimWww(h: string): string {
...
function urlFooterLabel(url: string): string {
  try {
    ...
    return trimWww(u.hostname) + path;
  } catch { return url; }
}
...
function CardTitle({ children, lines = 2 }: { children: React.ReactNode; lines?: number }) {
  return (
    <p style={{
      fontSize: 13, fontWeight: 600, color: "#dde0f0", lineHeight: 1.35, marginBottom: 3,
...
function CardDesc({ children }: { children: React.ReactNode }) {
  return (
    <p style={{
      fontSize: 11.5, color: "#4e5270", lineHeight: 1.4,
      overflow: "hidden", display: "-webkit-box",
  ...
  return (
    <div style={{
      ...
      <button
        ...
        style={{
          width: 16, height: 16, borderRadius: "50%", background: "#1a1d27",
    ...
    <a href={data.url} target="_blank" rel="noopener noreferrer" style={cardBase}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {faviconOk ? (
            ...
            <img
              src={faviconSrc} alt="" width={34} height={34}
        ...
        <div style={{ flex: 1, minWidth: 0, position: "relative", zIndex: 1 }}>
          <div style={{
            ...
            display: "flex", alignItems: "baseline", marginBottom: 7,
      ...
      {(data.title || data.description) && (
        <div style={{ padding: "11px 18px 13px" }}>
          ...
          {data.description && (
            <p style={{
              fontSize: 12, color: "#3e4260", lineHeight: 1.5,
              overflow: "hidden", display: "-webkit-box",
...
function VideoCard({ data, onDismiss }: { data: Preview; onDismiss: () => void }) {
  ...
  const host = getHostname(data.url);
  const ac = host.includes("twitch") ? "#9146ff" :
             host.includes("vimeo")  ? "#1ab7ea" : "#ff4444";

  return (
    <a href={data.url} target="_blank" rel="noopener noreferrer" style={cardBase}>
      <div style={{
        width: "100%", height: 196, position: "relative", overflow: "hidden",
        ...
        {data.image && !imgFailed && (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={data.image} alt="" loading="lazy" referrerPolicy="no-referrer"
            onError={() => setImgFailed(true)}
            style={{
              position: "absolute", inset: 0, width: "100%",
              height: "100%", objectFit: "cover", opacity: 0.85,
            }}
          />
        )}
        <div style={{
      ...
      <div style={{ display: "flex", padding: "10px 13px 12px", borderTop: `1px solid ${ac}22` }}>
        <div style={{
          width: 3, borderRadius: 2, flexShrink: 0, marginRight: 10, alignSelf: "stretch",
...
function ConnectCard({ data, onDismiss }: { data: Preview; onDismiss: () => void }) {
  return (
    <a href={data.url} target="_blank" rel="noopener noreferrer" style={cardBase}>
      <div style={{
        padding: "12px 14px", borderBottom: "1px solid #1a2226",
        ...
        <div style={{ flex: 1, minWidth: 0, position: "relative", zIndex: 1 }}>
          ...
          <p style={{
            fontSize: 10.5, fontWeight: 600, color: "#00d4c8",
            letterSpacing: ".05em", textTransform: "uppercase",
          }}>
            {data.siteName}&nbsp;&middot;&nbsp;&#1055;&#1088;&#1080;&#1075;&#1083;&#1072;&#1096;&#1077;&#1085;&#1080;&#1077;
          </p>
        </div>
        <div style={{
          background: "linear-gradient(135deg,rgba(0,212,200,.2),rgba(0,212,200,.08))",
          border: "1px solid rgba(0,212,200,.3)", borderRadius: 20,
          padding: "5px 13px", fontSize: 11, fontWeight: 700,
          color: "#00d4c8", letterSpacing: ".04em",
      ...
      {data.description && (
        <div style={{ padding: "9px 14px 11px", display: "flex", gap: 10, alignItems: "flex-start" }}>
          <div style={{
            width: 8, height: 8, borderRadius: "50%", background: "#23d160",
...
function AboutCard({ data, onDismiss }: { data: Preview; onDismiss: () => void }) {
  const [imgFailed, setImgFailed] = useState(false);
  const letter = (data.siteName || getHostname(data.url)).charAt(0).toUpperCase();

  return (
    <a href={data.url} target="_blank" rel="noopener noreferrer" style={cardBase}>
      <div style={{
        padding: "14px 16px 12px", borderBottom: "1px solid #1a1d2a",
        ...
        {data.image && !imgFailed ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={data.image} alt="" referrerPolicy="no-referrer"
            onError={() => setImgFailed(true)}
            style={{
        ...
        <div style={{ flex: 1, minWidth: 0, position: "relative", zIndex: 1 }}>
          ...
          <p style={{
            ...
            textTransform: "uppercase", color: "#8b7cf8",
          }}>
            {data.siteName}
          </p>
        </div>
        ...
        <span style={{
          background: "rgba(108,99,255,.14)", border: "1px solid rgba(108,99,255,.25)",
          borderRadius: 6, padding: "4px 9px", fontSize: 10, fontWeight: 700,
          color: "#9b94ff", letterSpacing: ".04em",
          flexShrink: 0, position: "relative", zIndex: 1,
        }}>
          &#1054;&nbsp;&#1085;&#1072;&#1089;
        </span>
        ...
        <div style={{
          ...
          {data.image && !imgFailed && (
            ...
            <img
              ...
              onError={() => setImgFailed(true)}
              style={{
                width: 56, height: 56, borderRadius: 7, objectFit: "cover",
...
function SkeletonCard() {
  return (
    <div style={{
      marginTop: 6, maxWidth: 440,
      background: "#13151e", border: "1px solid #1a1d27",
      ...
      <div style={{ padding: "10px 12px", flex: 1, display: "flex", gap: 10 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 7, paddingTop: 4 }}>
          ...
          <div className="tz-skeleton"
               style={{ height: 8, borderRadius: 4, width: "75%", animationDelay: ".15s" }} />
          <div className="tz-skeleton"
               style={{ height: 8, borderRadius: 4, width: "55%", animationDelay: ".3s" }} />
        </div>
      </div>
    </div>
  );
...
/** Первая ссылка в тексте сообщения. */
